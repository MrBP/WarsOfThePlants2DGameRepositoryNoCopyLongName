package net.mrblockplacer.WarsOfThePlants.render;

import java.awt.image.BufferedImage;
import java.io.IOException;

import javax.imageio.ImageIO;

public class SpriteSheets {

	private String path;
	public final int SIZE;
	public final int WIDTH, HEIGHT;
	public int[] pixels;

	public static SpriteSheets tiles = new SpriteSheets("/textures/sheets/spritesheet.png", 256);
	public static SpriteSheets spawn_level = new SpriteSheets("/textures/sheets/spawn_level.png", 48);

	public static SpriteSheets player = new SpriteSheets("/textures/sheets/DefaultPlayer.png", 128, 96);
	public static SpriteSheets player_down = new SpriteSheets(player, 0, 0, 1, 3, 32);

	private Sprite[] sprites;

	public SpriteSheets(SpriteSheets sheet, int x, int y, int width, int height, int spriteSize) {
		int xx = x * spriteSize;
		int yy = y * spriteSize;
		int w = width * spriteSize;
		int h = height * spriteSize;
		WIDTH = w;
		HEIGHT = h;
		// if (width == height) {
		// SIZE = width;
		// } else {
		SIZE = -1;
		// }
		pixels = new int[w * h];
		System.out.println(pixels.length + "  ::::");
		for (int y0 = 0; y0 < h; y0++) {
			int yp = yy + y0;
			for (int x0 = 0; x0 < w; x0++) {
				int xp = xx + x0;
				pixels[x0 + y0 * w] = sheet.pixels[xp + yp * sheet.WIDTH];
				System.out.println(pixels.length + "  ::::");
			}
		}
		int frame = 0;
		sprites = new Sprite[height * width];
		for (int ya = 0; ya < height; ya++) {
			for (int xa = 0; xa < width; xa++) {
				int[] spritePixels = new int[spriteSize * spriteSize];
				for (int y0 = 0; y0 < spriteSize; y0++) {
					for (int x0 = 0; x0 < spriteSize; x0++) {
						spritePixels[x0 + y0 * spriteSize] = pixels[(x0 + xa * spriteSize) + (y0 + ya * spriteSize) * WIDTH];

					}
				}
				Sprite sprite = new Sprite(pixels, spriteSize, spriteSize);
				sprites[frame++] = sprite;
			}
		}
	}

	public SpriteSheets(String path, int size) {
		this.path = path;
		SIZE = size;
		WIDTH = size;
		HEIGHT = size;
		pixels = new int[SIZE * SIZE];
		load();
	}

	public SpriteSheets(String path, int width, int height) {
		this.path = path;
		SIZE = -1;
		WIDTH = width;
		HEIGHT = height;
		pixels = new int[width * height];
		load();
	}

	public Sprite[] getSprites() {
		return null;

	}

	private void load() {
		try {
			BufferedImage image = ImageIO.read(SpriteSheets.class.getResource(path));
			int w = image.getWidth();
			int h = image.getHeight();
			image.getRGB(0, 0, w, h, pixels, 0, w);

		} catch (IOException e) {
			e.printStackTrace();
		}
	}

}
